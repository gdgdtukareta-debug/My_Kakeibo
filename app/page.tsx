...
import {
  Transaction, Subscription, TargetItem, AppMode,
  ThemeOption, BudgetMode, SurplusAction, Archives, NisaSettings
} from './types';
...
export default function Home() {
  ...
  const handleUpdateSettings = async () => {
    ...
    try {
      ...
      // バグ修正: targetItemがnullでもtempResetValuesに値がある場合は新規作成または無視しないようにする
      if (tempResetValues.targetPool !== undefined) {
        if (newTargetItem) {
          newTargetItem.currentAmount = tempResetValues.targetPool;
        } else if (tempResetValues.targetPool > 0) {
          newTargetItem = { name: "プール金ボックス", targetAmount: 0, currentAmount: tempResetValues.targetPool };
        }
      }
  ...
  const handleUpdateTransaction = async () => {
    ...
    try {
      ...
      const updatedHistoryItem: Transaction = {
        ...oldItem, // 重要: oldItemのフィールド（isSplit, investAmount, poolAmountなど）をすべて引き継ぐ
        amount: newAmount,
      ...
      <div className="max-w-md md:max-w-4xl mx-auto p-4">
        ...
        {isSettingMode ? (
          <SettingsModal
            ...
            isTacticsMode={isTacticsMode} setIsTacticsMode={setIsTacticsMode}
            surplusAction={surplusAction} setSurplusAction={setSurplusAction}
            targetItem={targetItem} setTargetItem={setTargetItem}
            tempIncomeInvestRatio={tempIncomeInvestRatio} setTempIncomeInvestRatio={setTempIncomeInvestRatio}
            totalMonthlyIncome={totalMonthlyIncome} setTotalMonthlyIncome={setTotalMonthlyIncome}
            calculateBudgetDistribution={calculateBudgetDistribution}
            livingBudgetMode={livingBudgetMode} setLivingBudgetMode={setLivingBudgetMode}
            dailyBudget={dailyBudget} setDailyBudget={setDailyBudget}
            monthlyLivingBudget={monthlyLivingBudget} setMonthlyLivingBudget={setMonthlyLivingBudget}
            payday={payday} setPayday={setPayday}
            monthlySavingTarget={monthlySavingTarget} setMonthlySavingTarget={setMonthlySavingTarget}
            monthlyInvestmentTarget={monthlyInvestmentTarget} setMonthlyInvestmentTarget={setMonthlyInvestmentTarget}
            tempResetValues={tempResetValues} setTempResetValues={setTempResetValues}
            nisaSettings={nisaSettings} setNisaSettings={setNisaSettings}
            subscriptions={subscriptions} setSubscriptions={setSubscriptions}
            theme={theme} setTheme={setTheme}
            isCsvMode={isCsvMode} setIsCsvMode={setIsCsvMode}
            isUnlimitedArchive={isUnlimitedArchive} setIsUnlimitedArchive={setIsUnlimitedArchive}
            secretCode={secretCode} setSecretCode={setSecretCode}
            downloadCSV={downloadCSV} handleUpdateSettings={handleUpdateSettings}
            setIsSettingMode={setIsSettingMode} setTacticsGuideType={setTacticsGuideType}
          />
        ) : (
                  ...
                  <div className="flex flex-wrap gap-2 mb-4 justify-center">
                    {currentCategories.map(cat => (
                      <button key={cat} onClick={() => setCategory(cat)}
                        className={`px-3 py-1.5 rounded-full text-[10px] font-bold transition-all ${category === cat
